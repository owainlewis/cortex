...
- Multi-character edits still mark content after a line break absorbed into a CRLF seam.
...
3. Track edited-line span separately from structural line-count contribution so absorbed CRLF seams still cover following content.
4. Keep the existing changed-range transition and final-newline rules unchanged.
5. Add renderer coverage that proves the gutter displays the corrected non-LF range.
6. Apply a focused improve pass to the changed code and tests without widening scope.
7. Review the final diff independently and address every valid finding.
